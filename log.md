...
* **Kit**: [docs/STATE_OF_PLAY.md](docs/STATE_OF_PLAY.md) — **option 3 recorded as the chosen direction** (port the generator's v0.2 engine into this kit, retire the generator as a builder), with the port's actual contents listed and the instruction to reconcile the two rule vocabularies first and on its own.
* **Kit**: [docs/STATE_OF_PLAY.md](docs/STATE_OF_PLAY.md) — **corrected an overstatement made an hour earlier.** It said the generator "has no ontology at all" and that its attempt "was judged to add nothing over this one". Both are wrong. The archived layer at tag `archive/ontology-attempt` is substantial (775 lines of `ontology.py`, 235 of `coverage.py`, a 107-line schema) and is a *different kind of artefact* — a typed-graph mechanism, where this kit's `ontology.md` is a governance registry. New §Reopened compares them on the merits. Two capabilities have no counterpart here: **instance resolution** (`identity`/`title_key`/`properties`, which its own note says is what an ontology of provisions and obligations needs — i.e. this corpus) and **competency-question coverage**, whose `out_of_scope` entries carry a stated reason, mechanising the D4 constraint that a reader must be able to explain why they are *not* seeing something. The rollback argument — markers described practice, typed edges legislated structure — still holds for the relationship layer and never covered the coverage layer. Measure the corpus before deciding, per D3/D4.

* **Kit**: [docs/STATE_OF_PLAY.md](docs/STATE_OF_PLAY.md) — corrected a claim that had gone stale within a day. It described `google-okf-generator` as "a spec-conformance reference, not a second builder — do not run corpora through it"; someone then applied it to a real corpus and hit two engine bugs. It is a builder in fact, so the doc now says so, and a new **§Open question — should the generator supersede the kit?** records the measured answer: **it cannot today.** The generator implements the format completely and the production system not at all — retiring the kit would break 104 projected concepts and both artefacts the live apps consume, 388 typed edges, 3 Coverage Ledgers, 12 Authority Posture rows, and the bundle's CI and `check.command`. Grep bears out the asymmetry (`projection` 0 files vs 23, `coverage` 0 vs 14, `authority posture` 0 vs 12). Supersede is a port, not a switch; three options are recorded with no decision taken.

* **Kit**: [scripts/okf_check.py](scripts/okf_check.py) — **projections were not reproducible across machines.** `Bundle` walked with `os.walk` and sorted `filenames` but not `dirnames`, so directory order came from the filesystem, which macOS and the Linux CI runner disagree about. `Bundle.files` was therefore machine-dependent and so was the edge order in a projection: the same corpus rebuilt in CI differed from the committed copy by **~1800 JSON lines with no concept changed**. Found only because the two earlier CI faults were fixed first — it was hidden behind them. `dirnames` is now sorted in place. Tests drive the walk with directory order **reversed** and assert the result is unchanged; reintroducing the bug fails them. 145 across the kit. D13 extended.
